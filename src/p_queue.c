...
/* 
    ...
    Removes the paren't each time it is not valid
*/
static void clean_resolved(pQueue q) {
    if (!q) return;
...
static void down(pQueue q, int i) {
    /* 
        We assume the actual node is the largest, and calculate the child indexes.
    */
    int largest_node = i; 
    int left_index = 2 * i;
    ...
    /* 
        We check if the left child exists via 'left_index <= n'.
    ...
    */
    ...
    /* 
        We check if the right child exists via 'right_index <= n'.
        If it is bigger than the parent node, we update it as the new parent.
    */
